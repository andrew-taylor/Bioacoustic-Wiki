...
Upload and manage obervations for the urban nature wiki.
...
package IkiWiki::Plugin::observations;
...
my $observations_defaults = 'observations_defaults';
...
my @metadata_field_whitelist
    = qw / date time latitude longitude location
           recordist recordingquality notes recording_notes/;

my $edit_action = 'edit_observation';
my $upload_action = 'upload_observations';
...
sub import {
    ...
    # Configuration
    hook(type => 'getsetup', id => 'observations', call => \&getsetup);
    hook(type => 'checkconfig', id => 'observations', call => \&checkconfig);

    # File upload
    add_underlay('javascript');
    hook(type => 'canrename', id => 'observations', call => \&canrename);
    hook(type => 'needsbuild', id => 'observations', call => \&needsbuild);
    hook(type => 'sessioncgi', id => 'observations', call => \&sessioncgi);

    IkiWiki::loadplugin('googlemaps');
    IkiWiki::loadplugin('filecheck');

    # Expand [[!observations]] directive
    hook(type => 'preprocess', id => 'observations', call => \&preprocess);
    hook(type => 'format', id => 'observations', call => \&format);
}
...
sub checkconfig () {
    if(! $config{rcs}) {
        error "The observations plugin really needs an RCS.";
    }
...
=pod
    ...
    Store the observation metadata in %wikistate. Add pointers from the
...
sub needsbuild (@) {
    ...
    # use Data::Dumper;
    # warn "observations::needsbuild start files: " . Data::Dumper::Dumper($files);

    if($config{rebuild}) {
        warn 'observations: dropping metadata on the floor.';
        delete $wikistate{observations};
    }
    ...
    foreach my $metadata_filename (grep { /\.$metadata_ext$/ } @{$files}) {
        ...
        my $f = sub {
            ...
            $pagestate{$page}{observations}{metadata}{$metadata_filename} = $type;
            ...
            if(-e "$config{srcdir}/$page_filename") {
                # warn "observations needsbuild: Adding dependency: $page -> $metadata_filename";
                ...
                warn "observations::needsbuild: file '$page_filename' does not exist.";
            }
        };
    ...
    # use Data::Dumper;
    # warn "observations::needsbuild end files: " . Data::Dumper::Dumper($files);
...
FIXME consider renaming user pages. If $pagestate{src} has observation
metadata links...

FIXME can't rename KML files or observation metadata.
...
sub canrename {
    ...
    if($src =~ m!/species/!) {
        ...
        # Move the observation metadata across.
        $pagestate{$dest}{observations} = $pagestate{$src}{observations};

        foreach my $metadata_filename (keys %{$pagestate{$src}{observations}{metadata}}) {
            my $metadata = $wikistate{observations}{metadata}{$metadata_filename};
            die "observations::canrename: Patching up '$metadata_filename', '$src' becomes '$dest'";
        }
...
in $config{allowed_observations}.
...
sub show_edit_form {
    ...
    my $p = { title => 'Edit obervation metadata',
              action => $edit_action,
              buttons => ['Save'],
...
sub show_upload_form {
    ...
    my $metadata = IkiWiki::userinfo_get($session->param('name'), $observations_defaults);
    if($metadata) {
        metadata_add_render_info('FIXME page', $metadata);
    } else {
    ...
    my $p = { title => 'Upload observation',
              action => $upload_action,
              buttons => ['Upload observation'] };
...
sub show_form {
    ...
    my $form = CGI::FormBuilder->new(
        ...
        template => { template('observationsupload.tmpl') },
       );
    ...
    $javascript .=
        "<script src=\"$config{url}/observationsupload.js\" type=\"text/javascript\"></script>\n";
    ...
    $form->field(name => 'recording_notes', type => 'textarea', validate => $notes_re, label => 'Recording Notes:');
    ...
    $form->field(name => 'recording_notes', value => decode_entities($metadata->{'recording_notes'}));
    ...
    if($form->submitted && $form->validate) {
        ...
        if($q->param('setdefaults')) {
            ...
            IkiWiki::userinfo_set($user, $observations_defaults, $metadata);
        }
        ...
        $metadata->{'recording_notes'} = encode_entities($metadata->{'recording_notes'});

        my $msg;
        if($p->{action} eq $upload_action) {
            # Handle the multiple-file upload.
            # FIXME probably should cope with the non-Javascript case
            # (having 'observations') too.
            warn "observations: Processing uploaded files...";

            foreach my $param ($q->param) {
                next unless $param =~ /observation_(\d+)/;
                ...
                $msg = "Uploaded observation '$srcfile' -> '$metadata_canonical_filename'";

                add_user_observations_directive($user, $metadata_canonical_filename);
            }
        } else {
            edit_uploaded_file($session, $form, $metadata);
            $msg = "Edited metadata of observations '$p->{metadata_filename}'";
        }
...
it that will list their observations.
...
sub add_user_observations_directive {
    ...
    warn "add_user_s_directive: '$user' '$user_filename_canonical'";

    # FIXME robustify
    if(!-e $user_filename_canonical) {
        warn "add_user_observations_directive: creating page '$user_filename'";

        my $tmpl = template("observations_user.tmpl", blind_cache => 1);
        IkiWiki::writefile($user_filename, $config{srcdir}, $tmpl->output);
        IkiWiki::rcs_add($user_filename);
    }

    # FIXME assume the user has a observations directive.
    # FIXME could use a pagespec here rather than individual files.
    warn "add_user_observations_directive: Adding dependency: $user_page -> $metadata_canonical_filename";
...
########################################
# The preprocessor directive [[!observations]]

sub preprocess {
    my %p = @_;
    my $page = $p{page};

    my $tmpl = template_depends("observations.tmpl", $page, blind_cache => 1);
...
sub include_javascript {
    ...
    $js = "<!--observations.pm-->\n".observations_js($page);
    my $tax_js = urlto(IkiWiki::Plugin::taxonomy::description_filename(), '', 1);
    $js .= <<EOF;
...
<script src="$tax_js" type="text/javascript"></script>
<script type="text/javascript">var soundfiles_ikiwiki_baseurl="$config{url}";</script>
<script src="$config{url}observations.js" type="text/javascript"></script>
<script type="text/javascript">var one_bit_url = "$config{url}1bit.swf";</script>
<script src="$config{url}1bit.js" type="text/javascript"></script>
EOF
    return $js;
}
...
FIXME Generate the javascript for the observations for the taxonomy,
species and user pages.

=cut
my %observations_memo;

sub observations_js {
    my ($page) = @_;

    if($page =~ m!/species/!) {
        return observations_js_for_page(type => 'species',
                                      page => $page,
                                      observations => $pagestate{$page}{observations}{metadata});
    } elsif($page =~ m!/taxonomy!) {
        # Propagate the observations information upwards.
        my $node_fn = sub {
            my %p = @_;
            my %sfs;
        ...
        my $species_fn = sub {
            ...
            return $pagestate{$page}{observations}{metadata} || {};
        };

        # Populate the memo table.
        IkiWiki::Plugin::taxonomy::traverse_taxonomy_tree($page, $species_fn, $node_fn, \%observations_memo);

        #use Data::Dumper;
        #warn "observations_js / memo '$page': " . Data::Dumper::Dumper($soundfiles_memo{$page});

        return observations_js_for_page(type => 'taxa',
                                      page => $page,
                                      observations => $observations_memo{$page});
    } else {
        # User pages.
        my $observations = $pagestate{$page}{observations}{metadata};

        # use Data::Dumper;
        # warn "observations_js user page '$page' " . Data::Dumper::Dumper($pagestate{$page}{observations});

        if($observations) {
            return observations_js_for_page(type => 'user',
                                          page => $page,
...
sub observations_js_for_page {
    ...
    if($type eq 'user' || $type eq 'taxa') {
        ...
        foreach my $metadata_filename (keys %{$p{observations}}) {
            my $metadata = $wikistate{observations}{metadata}{$metadata_filename};
        ...
        # use Data::Dumper;
        # warn "observations_js_for_page($p{page}): " . Data::Dumper::Dumper(\@markers);
        ...
        #use Data::Dumper;
        #warn "observations_js_for_page($p{page}): " . Data::Dumper::Dumper($p{observations});
        ...
        foreach my $metadata_filename (keys %{$p{observations}}) {
            ...
            if($p{observations}->{$metadata_filename} eq $foreground) {
                push @fg_markers, $metadata;
            } elsif($p{observations}->{$metadata_filename} eq $background) {
...
=pod
    ...
    my $observations_count = count_observations($genus, $species);

Counts the number of observations (foreground and background) for the
given species.
...
sub count_observations {
    ...
    foreach my $type (values %{$pagestate{$page}{observations}{metadata}}) {
